...
## 보스 밸런스 — 체력이 아니라 '버티는 시간'으로 맞춥니다

보스 4종(Alpha Hybrid, Wyrm Titan, Megarachne, Malformed Rex)은 헌터 10마리를 상대로
**아슬아슬하게 이기거나 지도록** 맞춰져 있습니다. 스탯 블록만 보면 체력 수치가 이상해 보이는데,
이유가 있습니다.

**방어력은 비율이 아니라 정액 차감입니다** (`Health.TakeDamage`: `max(1, raw - armor)`).
그래서 약한 공격자에게 훨씬 강력합니다 — 방어력 78은 랩터의 110 물기에서 71%를 깎지만
Bio T-Rex의 560에서는 14%만 깎습니다. 체력과 방어력은 독립된 손잡이가 아닙니다.

실제로 승패를 결정하는 값은 **헌터 10마리가 보스를 갈아내는 데 걸리는 시간**입니다.
8전씩 측정한 결과가 이걸 거의 그대로 예측했습니다:
...
반대로 **헌터를 가장 빨리 죽이는 보스가 거의 모든 판을 졌습니다.** 10마리가 온전히 살아 있는
팩의 화력은 무엇으로도 못 이기고, 오래 버티는 것이 곧 그 팩을 줄이는 유일한 방법이기 때문입니다.
즉 공격력보다 내구력이 지배적입니다.

그래서 넷 다 **버티는 시간 약 24초**로 맞췄고, 체력 수치는 거기서 파생된 결과입니다.
방어력이 낮은 거미가 방어력 높은 Malformed Rex보다 체력이 더 많은 건 그 때문입니다.
**방어력을 건드리면 체력을 반대 방향으로 같이 옮겨야 합니다.**

### 이 측정의 한계 — 보스별 8전으로는 승률을 못 맞춥니다

거미가 다른 셋보다 약해 보여서(같은 내구력에서 16전 3승) 체력을 더 줬더니 2/8 → 6/8 로 올랐습니다.
그런데 **같은 판에서 스탯을 하나도 안 건드린 나머지 셋이 4/8 → 2/8, 5/8 → 1/8, 3/8 → 1/8 로
같이 움직였습니다.** 안 바꾼 대상이 그만큼 흔들린다면 그 측정으로는 바꾼 대상을 판단할 수 없습니다.
그래서 거미 체력은 원래의 등가 내구력 값으로 되돌렸습니다.

흔들리는 이유는 **팩 구성이 매판 무작위**이기 때문입니다 (5종에서 10마리 추첨).
정액 방어력이라 랩터가 몇 마리 뽑히느냐가 방어력 낮은 보스에게 특히 크게 작용합니다.
`Time.timeScale = 16` 에서 도는 것도 프레임 사정에 따라 판마다 물리 결과가 달라질 수 있습니다.

**따라서 승률은 ±25%p 수준까지만 신뢰하세요.** 반면 **접전 비율은 안정적입니다** — 어느 판을
돌려도 절반 넘게 "보스 체력 한 자릿수 %" 또는 "팩 1~2마리 생존"으로 끝납니다. 원래 목표가
아슬아슬한 싸움이므로 튜닝은 승률이 아니라 이쪽을 보고 하세요.

승률까지 정밀하게 맞추려면 `BattlesPerBoss` 를 크게 올려야 합니다.

사거리와 공격 기회는 원인이 아님을 측정으로 배제했습니다 — 거미는 가장 짧은 사거리에 가장 넓은
몸인데도 사거리 안에 적이 있는 시간이 97%로 나머지와 같았고, 공격 횟수는 오히려 가장 많았습니다
(21회 대 10~13회).

밸런스를 다시 만질 때는 `Dino Battle > Advanced > Probe Boss Balance` 를 돌리세요.
보스별로 8전씩 자동으로 굴리고 승패·접전 비율을 보고합니다. **보스별로 나눠서 봐야 합니다** —
예전처럼 무작위로 섞어 12전을 돌리면 종합 5:7로 멀쩡해 보이는데, 쪼개 보니 한 종이 1:7이었습니다.
